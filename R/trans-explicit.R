...
#'
crs_trans_explicit <- function(value, use_z = NA, use_m = NA) {
  value <- wk::as_xy(value)
  value <- wk::as_xy(value, dims = c("x", "y", "z", "m"))
  wk::new_wk_trans(
    .Call(crs2crs_c_trans_explicit_new, value, as.logical(use_z)[1], as.logical(use_m)[1]),
    "crs2crs_trans_explicit"
  )
}
