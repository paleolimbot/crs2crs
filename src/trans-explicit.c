...
typedef struct {
  ...
  R_xlen_t i;
  R_xlen_t n;
} crs2crs_trans_explicit_t;

int crs2crs_trans_explicit_trans(R_xlen_t feature_id, const double* xyzm_in, double* xyzm_out, void* trans_data) {
  crs2crs_trans_explicit_t* data = (crs2crs_trans_explicit_t*) trans_data;
  data->i++;
...
void crs2crs_trans_explicit_finalize(void* trans_data) {
  free(trans_data);
}
...
SEXP crs2crs_c_trans_explicit_new(SEXP xy, SEXP use_z, SEXP use_m) {
  ...
  // create the crs2crs_trans object
  wk_trans_t* trans = wk_trans_create();
  trans->trans = &crs2crs_trans_explicit_trans;
  trans->finalizer = &crs2crs_trans_explicit_finalize;

  crs2crs_trans_explicit_t* data = (crs2crs_trans_explicit_t*) malloc(sizeof(crs2crs_trans_explicit_t));
  if (data == NULL) {
    free(trans); // # nocov
    Rf_error("Failed to alloc crs2crs_trans_explicit_t"); // # nocov
  }
